...
notFound(AsyncWebServerRequest *request) {
    if (!dis_sd) {
        ...
        if (request->url().endsWith(F(".txt")) ||
            (request->url().endsWith(F(".csv"))) ||
            (request->url().endsWith(F(".TXT"))) ||
            (request->url().endsWith(F(".CSV")))) {
...
async_server_on() {
    ...
    server.on("/settings", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (!request->authenticate(http_username.c_str(),
                                   http_password.c_str()))
    ...
    server.on("/iSerial", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (!request->authenticate(http_username.c_str(),
                                   http_password.c_str()))
            return request->requestAuthentication();
...
void
store_value(const char *store_temp, uint8_t store_int) {
...
void
store_toggle(const char *store_temp, bool store_bool) {
    preferences.begin("stored_values", false);
...
void
onEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type,
...
printDirectory(File dir) {
    ...
    while (true) {
        ...
        if (file_name.startsWith("/.") || (file_name.indexOf("_") > 0) ||
            (file_name.startsWith("/System"))) {
        } else if (entry.isDirectory()) {
            file_name.remove(0, 1);
            response += String("<a href='") + String(entry.name()) +
                        String("'>") + String(file_name) + String("</a>") +
                        String("</br>");
        } else {
            file_name.remove(0, 1);
            response += String("<a href='") + String(entry.name()) +
