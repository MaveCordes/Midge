...
void
handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
    ...
    if (info->final && info->index == 0 && info->len == len &&
        ...
        if (message.indexOf("hum_target") >= 0) {
            ...
            String input_value = message.substring(message.indexOf("&1") + 2,
                                                   message.indexOf("%"));
            String input_value2 =
                message.substring(message.indexOf("&2") + 2, message.length());
        ...
        if (strcmp((char *) data, "toogle_light") == 0) {
            ...
            ESP.restart();
        } else if (strcmp((char *) data, "toggle_sd") == 0) {
            sd = !sd;
            store_toggle("sd", sd);
            ESP.restart();
        }
